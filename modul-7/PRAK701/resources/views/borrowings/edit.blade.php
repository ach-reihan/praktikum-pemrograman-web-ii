...
@section('title', 'Ubah Transaksi Peminjaman - Perpustakaan Reihan')

@section('content')
<div class="max-w-xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <h3 class="text-2xl font-extrabold text-slate-800">Ubah Transaksi Peminjaman</h3>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
        <form action="{{ route('borrowings.update', $borrowing->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            
            <div>
                <label for="member_name" class="block text-sm font-semibold text-slate-700 mb-1.5">Nama Member</label>
                <input type="text" id="member_name" name="member_name" value="{{ old('member_name', $borrowing->member_name) }}" 
            ...
            <div>
                ...
                <input type="date" id="borrow_date" name="borrow_date" value="{{ old('borrow_date', date('Y-m-d', strtotime($borrowing->borrow_date))) }}" 
            ...
            <div>
                ...
                <input type="date" id="return_date" name="return_date" value="{{ old('return_date', date('Y-m-d', strtotime($borrowing->return_date))) }}" 
